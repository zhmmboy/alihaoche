...
; (function ($) {
    ...
    */
    $('.avatar').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/avatar-default.png',
        threshold: 400
    });
    ...
    $('.carousel .thumb').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/thumbnail.png',
        threshold: 400
    });

    $('.likepage .thumb').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/thumbnail.png',
        threshold: 400
    });
    ...
    $('.posts-xs .thumb').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/thumbnail.png',
        threshold: 400
    });

    $('.posts-medium .thumb').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/thumbnail.png',
        threshold: 400
    });

    $('.posts-thumbnail .thumb').lazyload({
        data_attribute: 'src',
        placeholder: jsui.uri + '/res/image/thumbnail.png',
        threshold: 400
    });
    ...
    //    pagination: '.pagination',
    //    next: '.next-page a',
    //    loader: '<div class="pagination-loading"><img src="' + jsui.uri + '/res/image/loading_line.gif"></div>',
    //    trigger: '下一页',
    //    onRenderComplete: function () {
    ...
    */
                    ...
                    <div class="modal-dialog" style="margin-top:200px;width:340px;">\
                        <div class="modal-content">\
                            <div class="modal-header">\
                                ...
                                <h4 class="modal-title">关注公众号 “阿里好车”</h4>\
                            </div>\
                            <div class="modal-body" style="text-align:center">\
                                <img src="' + jsui.uri + '/res/image/weixin-qrcode.jpg">\
                            </div>\
                        </div>\
                    </div>\
                </div>\
    ...
    $('.widget_ui_viewposts:first').affix({
        offset: {
    ...
    */
    /*var sb = $('.sidebarfix')
    var pcs = $('.sidebarfix #postcomments')
    ...
    function winsb(){
        var sbc = sb.css('content')
        if( sbc !== "'1024'" && pcs.length ){
            ...
            $('.sidebarfix').on('affix-bottom.bs.affix', function(){
                pcsheight()
            })

            $('.sidebarfix').on('affix.bs.affix', function(){
                pcsheight()
            })
        }else{
            pcs.height('auto')
    ...
    function pcsheight(h){
        pcs.length && pcs.height( $(window).height()-pcs.position().top-78-30+(h||0) )
    }*/
    ...
    */
    ...
    if( _side.length ){
        ...
        _widget.on('affix.bs.affix', function(){
            _widget.css({top: $('.header').height()+15}) 
        })
    }*/

    /*if( _side ){
        var roller = [1, 2]
        var rollFirst = _side.find('.widget:eq('+(Number(roller[0])-1)+')')
        var sheight = rollFirst.height()
        rollFirst.on('affix-top.bs.affix', function(){
            rollFirst.css({top: 0}) 
            sheight = rollFirst.height()

            for (var i = 1; i < roller.length; i++) {
                var item = Number(roller[i])-1
                var current = _side.find('.widget:eq('+item+')')
                current.removeClass('affix').css({top: 0})
            };
        })

        rollFirst.on('affix.bs.affix', function(){
            rollFirst.css({top: 92}) 

            for (var i = 1; i < roller.length; i++) {
                var item = Number(roller[i])-1
                var current = _side.find('.widget:eq('+item+')')
                current.addClass('affix').css({top: sheight+100})
                sheight += current.height() + 82
            };
        })
    ...
    */
    ...
    $('.widget_comments a, .readers a').tooltip({
        container: 'body',
        placement: 'top'
    })
    ...
    */
    ...
    window._bd_share_config = {
        ...
        share: [{
            // "bdSize": 12,
            bdCustomStyle: jsui.uri + '/res/css/share.css'
        }],
        /*slide : {    
            bdImg : 4,
            bdPos : "right",
    ...
    */
    ...
    function logtips(str) {
        if (!str) return false; _loginTipstimer && clearTimeout(_loginTipstimer); $('.login-tips').html(str).animate({
            top: 0
        },
		220);
    ...
    window._is_login = $('body').hasClass('logged-in') ? true : false;
	...
	function (e) {
	    ...
	    switch (type) {
	        ...
	        case 'like':
	            var pid = _ta.attr('data-pid'); //if (!pid || !/^\d{2,8}$/.test(pid)) return;
	            ...
	            $.ajax({
	                url: jsui.uri + '/res/action/Handler.ashx',
	                type: 'POST',
	                dataType: 'json',
	                data: {
	                    type: dataType,
	        ...
	        case 'comment-user-change':
	            $('#comment-author-info').slideDown(300); $('#comment-author-info input:first').focus();
	        ...
	        case 'login':
	            ...
	            if (!window._loginCache) {
	                $.ajax({
	                    type: 'POST',
	                    url: jsui.uri + '/res/action/loginweb.aspx',
	                    success: function (data) {
	                        window._loginCache = true; $('body').append(data); $('#modal-login').on('shown.bs.modal',
                            function (e) {
	        ...
	        case 'login.submit':
	            ...
	            if (!is_mail(inputs.email)) {
	                logtips('邮箱格式错误'); return;
	            }

	            if (inputs.password.length < 6) {
	                logtips('密码太短，至少6位'); return;
	            }

	            if (isreg) {
	                if (!is_name(inputs.name)) {
	                    logtips('昵称限制在3-20字'); return;
	                }
	            }

	            $.ajax({
	                type: "POST",
	                url: jsui.uri + '/res/action/login.ashx',
	                data: inputs,
	                dataType: 'json',
	                success: function (data) {
	                    if (data.error) {
    ...
    */
    ...
    $('#commentform').submit(function () {
        ...
        $.ajax({
            url: jsui.uri + '/res/action/comment.ashx',
            data: $(this).serialize(),
